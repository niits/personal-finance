...
Refund category, budget period, and payment medium are derived from the original expense. Refund expense metrics are grouped by `budget_period_id`, not by the refund's cash date. For each partial refund, compute the newly refunded amount as cumulative refund minus all prior releases, then allocate only that increment proportionally across each custom allocation's remaining capacity and the remaining unallocated bucket. Use largest-remainder rounding with ascending custom-budget ID as the stable tie-breaker and sort the unallocated bucket last. This incremental rule is monotonic: a later refund never reclaims an earlier release, every custom delta stays negative, and a full cumulative refund exhausts every original allocation and the unallocated remainder exactly. Corrupt or decreasing prior release state is rejected. The client does not provide refund allocations.
