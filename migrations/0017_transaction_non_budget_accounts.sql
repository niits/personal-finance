-- Fixed debt/savings categories are ordinary expense transactions but must not
-- require a monthly budget. The earlier CHECK predates finance_account_id.
...
CREATE TABLE "transaction_new" (
  ...
  credit_card_id     TEXT REFERENCES credit_card(id) ON DELETE SET NULL,
  created_at         INTEGER NOT NULL DEFAULT (unixepoch()),
  updated_at         INTEGER NOT NULL DEFAULT (unixepoch()),
  CHECK (
    type = 'income' OR monthly_budget_id IS NOT NULL OR debt_id IS NOT NULL OR finance_account_id IS NOT NULL
  )
);

INSERT INTO "transaction_new" (id, user_id, amount, type, category_id, note, emoji, date, monthly_budget_id, debt_id, linked_amount, finance_account_id, credit_card_id, created_at, updated_at)
  SELECT id, user_id, amount, type, category_id, note, emoji, date, monthly_budget_id, debt_id, linked_amount, finance_account_id, credit_card_id, created_at, updated_at
  FROM "transaction";
