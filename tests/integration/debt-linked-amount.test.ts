...
let cookie: string; let userId: string; let categoryId: number; let cardId: string;
beforeAll(async () => {
  ...
  const card = await SELF.fetch("http://localhost/api/credit-cards", { method: "POST", headers: authHeaders(cookie), body: JSON.stringify({ group_id: createdGroup.id, name: "Visa" }) });
...
describe("card purchase, budget, and statement payment", () => {
  it("counts a card purchase once in consumption and the unpaid subset", async () => {
    const purchase = await SELF.fetch("http://localhost/api/transactions", { method: "POST", headers: authHeaders(cookie), body: JSON.stringify({ amount: 250_000, type: "expense", category_id: categoryId, credit_card_id: cardId, date: "2026-05-10", note: "Siêu thị" }) });
